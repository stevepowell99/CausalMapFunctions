...
e3  %>%
  ...
  pipe_trace_continuity() %>% 
  pipe_bundle_links() %>% 
  pipe_label_links(field = "is_continued_after",fun = "sum") %>% 
  pipe_color_links(field = "is_continued_after",fun = "sum") %>% 
  pipe_color_borders(field="n_unique_from_before_continued") %>% 
  make_print_map() 
...
e3  %>%
  ...
  pipe_trace_continuity() %>% 
  pipe_bundle_links() %>% 
  pipe_label_links(field = "continued_after_sid",fun = "literal") %>% 
  pipe_color_links(field = "is_continued_after",fun = "sum") %>% 
  pipe_color_borders(field="n_unique_from_before_continued") %>% 
  make_print_map() 
...
e3  %>%
  ...
  pipe_trace_continuity() %>% 
  pipe_bundle_links() %>% 
  pipe_label_links(field = "continued_after_sid",fun = "count") %>% 
  pipe_color_links(field = "is_continued_after",fun = "sum") %>% 
  pipe_color_borders(field="n_unique_from_before_continued") %>% 
  make_print_map() 
...
# note that sum of is_continued_after can be more than number of unique sources
oo %>%    
  pipe_select_factors(10) %>% 
  pipe_trace_paths(from = "Learnt about farming",to="Health",length = 3) %>% 
  pipe_trace_continuity() %>% 
...
kb %>%    
  ...
  pipe_trace_continuity() %>% 
  pipe_bundle_links() %>% 
  pipe_label_links(field = "continued_after_sid",fun = "initials") %>% 
  make_print_map
