...
## Recalculating - shouldn't make any difference now as factor and link fields are recalculated after every transform; but before and afterids are added only on load_mapfile.
...
e3  %>%
  ...
  pipe_bundle_links() %>% 
  pipe_label_links(field = "is_continued",fun = "sum") %>% 
  pipe_color_links(field = "is_continued",fun = "sum") %>% 
  make_print_map() 
...
e3  %>%
  ...
  pipe_color_borders(field="n_incoming_continued") %>% 
  make_print_map() 
...
# note that sum of is_continued can be more than number of unique sources
oo %>%    
  pipe_select_factors(10) %>% 
  pipe_trace_paths(from = "Learnt about farming",to="Health",length = 3) %>% 
  pipe_trace_continuity() %>% 
