...
```{r}
mm %>% 
  parse_commands("find factors field=label value=(AG) Farm production operator=contains up=1 down=1
combine opposites
bundle links group=flipped_bundle
scale links field=source_id fun=count
label links field=source_id fun=count
wrap factors length=22
color factors field=found lo=white hi=#7FC97F") %>% .$links %>% select(flipped_bundle,from_flipped,to_flipped)
