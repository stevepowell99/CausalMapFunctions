...
#  ee <- load_mapfile("example2") #%>% pipe_recalculate_all()
  ee <- example2 %>% pipe_coerce_mapfile()
...
graf <- ee
tic();graf$factors <- graf$factors %>% arrange(size);graf$links <- graf$links %>% arrange(link_id);toc()
tic();graf %>% pipe_update_mapfile(factors <- graf$factors %>% arrange(size),links <- graf$links %>% arrange(link_id));toc()
