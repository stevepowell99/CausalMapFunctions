...
## a rewrite of the one in Functions pkg
#' Recode Factors in a Pipe
#'
#' Recodes factor levels in a graph using specified recodes or generated recodes from embeddings.
#'
#' @param graf The input graph object.
#' @param recodes Optional recodes as a data frame. If not provided, recodes are generated from embeddings.
#' @param top_level_only Logical value indicating whether to recode only top-level factor levels. Default is \code{TRUE}.
#' @param retain_unmatched Logical value indicating whether to retain unmatched factor levels in the recoding. Default is \code{TRUE}.
#' @param slider_strictness The strictness level for generating recodes from embeddings. Default is 50.
#' @param wordify Logical value indicating whether to wordify recoded factor levels. Default is \code{FALSE}.
#'
#' @return The graph object with recoded factor levels.
#'
#'
#' @examples
#' # Example usage
#' pipe_recode_factors(example_file, recodes = tibble(old="Farm production",new = "xx"))
#'
#' @export
pipe_recode_factors <- function (graf, recodes=NULL,top_level_only = T,
                                 retain_unmatched = T,slider_strictness=50,wordify=F)
{


  # browser()
  if(is_tibble(graf$settings)) graf$settings <- NULL

  strictness <-
    2-(slider_strictness*.015)

  if(is.null(recodes)){
    if(is.null(graf$settings$recodes)){
      if(is.null(graf$settings$embeddings))graf <- graf %>% pipe_add_embeddings()
      recodes <-
        graf$settings$embeddings %>%
        generate_recodes_from_embeddings(height=strictness)
      graf$settings$recodes <- recodes
      graf$settings$strictness <- slider_strictness
      graf$settings$cluster_method <- "embeddings"
    }
    if(wordify){

      graf <- graf %>% pipe_wordify_recodes()
      recodes <- graf$settings$recodes
    }
  ...
  from_labels <- graf$links$from_label
  to_labels <- graf$links$to_label
  if (top_level_only) {

    from_labels <- from_labels %>% keep_top_level()
    to_labels <- to_labels %>% keep_top_level()
  }

  new_from_labels <- recode(from_labels, !!!(recodes$new %>% set_names(recodes$old)),missing = ifelse(retain_unmatched, recodes$old, NULL))
  new_to_labels <- recode(to_labels, !!!(recodes$new %>% set_names(recodes$old)),missing = ifelse(retain_unmatched, recodes$old, NULL))

  if (top_level_only) {
    new_from_labels <- paste0(new_from_labels, ";", from_labels %>% (drop_top_level)) %>% str_remove_all(";$")
    new_to_labels <- paste0(new_to_labels, ";", to_labels %>% (drop_top_level)) %>% str_remove_all(";$")
  }
  graf$links$from_label <- new_from_labels
  graf$links$to_label <- new_to_labels

  graf %>% pipe_reconstruct_factors_from_links() %>% pipe_coerce_mapfile

}
get_top_words <- function(tx){
  ww2 <- tx %>% unlist %>% str_split(" ") %>% pluck(1) %>% table %>% sort
  ww2[setdiff(names(ww2) , stopwords(source="snowball"))] %>% tail(5) %>% rev %>% names %>% collap(", ")
}
pipe_wordify_recodes <- function (graf, recodes = graf$settings$recodes)
{
  if (is.null(recodes)) {
    warning("You did not provide recodes")
    return(graf)
  }

  graf <-
    graf %>%
    pipe_wordify_factors()
}

pipe_wordify_factors <- function (graf, recodes = graf$settings$recodes)
{
  if (is.null(recodes)) {
    warning("You did not provide recodes")
    return(graf)
  ...
  tab <-
    recodes %>%
    group_by(new) %>%
    summarise(words=paste0(old,collapse=" "))


  # ask_AI("Find a label which best expresses the content of the following ideas\n",recodes %>% filter(new=="systemic racism") %>% pull(old) %>% collap)
  ...
  recodes <-
    recodes %>%
    left_join(tab) %>%
    rename(old_recode=new,new=new_words) %>%
    unite("new",old_recode,new,sep = ": ")
...
find_example_statements <- function(){
  if(is.null(sess$graf))return()

  sess$graf$statements %>%
    filter(str_detect(statement_memo,"!EXAMPLE")) %>%
    pull(statement_id)
}
...
get_first_line <- function(tx){
  str_extract(tx,"[^\n]*\n")
}

pipe_NLPidentify_links <- function(graf=graf,start=start,auto_coding_one_batch=F,toplevel=NULL,auto_coding_n_statements=1,sentences_per_batch=50,presence_penalty=0,frequency_penalty=0){
  resp <-
    NLP_get_response(df=graf$statements,
                     start=start,
                     toplevel=toplevel,
                     auto_coding_one_batch=auto_coding_one_batch,
                     auto_coding_n_statements=auto_coding_n_statements,
  ...
  resp <- resp %>%
    mutate(answer=str_remove(answer,"^[0-9]*\\. *"))
  ...
  links <-
    ...
    map(function(j){
      resp[j,"answer"] %>%
        ...
        str_split(" >> ") %>%
        pluck(1) %>%
        chain_to_links(resp[j,"statement_id"] %>% unlist)
    }) %>%
    bind_rows

  #  browser()
  if(nrow(links)==0){
    notify("No links identified",3)
    return(graf)}

  tmp <-
    factors_links_from_named_edgelist(links %>% rename(from_label=from,to_label=to))

  graf %>%
    pipe_update_mapfile(links=tmp$links,factors=tmp$factors) %>%
    # pipe_coerce_mapfile() %>%
    add_attribute(resp,"resp")


}


make_prompt <- function(prompts,presence_penalty=0,frequency_penalty=0,model="4"){
  # browser()
  row_index(prompts) %>%
    map(function(i){
      # browser()
...
chunkify <- function(prompt,sentences_per_batch=50){
  # browser()
  prompt %>%
    unite(text,statement_id,text,sep = ":\n",remove = F) %>%
    mutate(text=paste0("Statement ",text))

  # prompt %>%
  #   str_split("\\. *|\n") %>%
  #   pluck(1) %>%
  #   tibble(text=.) %>%
  #   # mutate(nsentences=nsentences(text)) %>%
  #   mutate(cumnsentences=row_number()) %>%
  #   mutate(cumnsentences2=cumnsentences%%sentences_per_batch) %>%
  #   # mutate(cumnsentences2=cumnsentences%%1000) %>%
  #   mutate(new=cumnsentences2<lag(cumnsentences2,default = 0)) %>%
  #   mutate(statement_id=cumsum(new)+1) %>%
  #   group_by(statement_id) %>%
  #   summarise(text=paste0(text,collapse="\n"),cumnsentences=max(cumnsentences))

}

ask_AI <- function(prompt,text_vec="",auto_coding_one_batch=T,coding_model=coding_model){
  NLP_get_response(tibble(text=text_vec %>% collap,statement_id=1,auto_coding_one_batch=auto_coding_one_batch),start=prompt)
}
NLP_get_response <- function(df,start,toplevel=NULL,auto_coding_one_batch=F,sentences_per_batch=NULL,auto_coding_n_statements=1,presence_penalty=0,frequency_penalty=0){
  # browser()
  if(
    is.null(start)){message("fail")
    return()
  }
  if(is.null(sentences_per_batch))
    tx <- df else
    {
      tx <-
        df  %>%
        chunkify(sentences_per_batch=sentences_per_batch)

    }


  if(auto_coding_one_batch)tx <- tx %>% group_by(statement_id>0) %>% select(text,statement_id) %>% summarise_all(collap)
  prompts <-
    tx %>%
    select(statement_id,text) %>%
    mutate(text=paste0(start,"\n",text,"\n\nAnswer:\n"))
  # tibble(text=paste0(start,"\n",tx,"\n\nAnswer:\n"),id=tx$statement_id)

  resps=NULL
  all_statements <- NULL
  fail <- 1
  # stats <- 0
  # oldstats <- prompts$statement_id # so it doesn't keep getting stuck on 1 item and repeat to end of counter
  counter <- 1

  slice_size=100
  counter_limit <- max(5,5*nrow(prompts)/slice_size)

  while(nrow(prompts)>0  & counter<counter_limit ){
    counter <- counter+1
    # if (counter==10)browser()

    reqs <-make_prompt(prompts %>% slice(1:slice_size),presence_penalty=presence_penalty,frequency_penalty=frequency_penalty,model=input$coding_model)
    # browser()
    respsi <- multi_req_perform(reqs)

    statements <- prompts$statement_id %>% head(slice_size)

    message(paste0("Getting response ",counter))
    message(paste0("nrow ",nrow(prompts)))
    ...
    if(sum(fail)>0){message(paste0("Requests not complete: ",paste0(which(fail),collapse=", ")),3)}
    respsi <- respsi[!fail]
    ...
    prompts <-
      prompts %>% filter(statement_id %notin% statements)
    # oldstats <- prompts$statement_id # so it doesn't keep getting stuck on 1 item and repeat to end of counter
  ...
  tmp <-
    seq_along(resps) %>%
    map(function(s){
      resp_body_json(resps[[s]])$choices[[1]]$message$content %>% str_split("\n") %>% pluck(1) %>% tibble(answer=.,statement_id = all_statements[s]) %>%
        filter(answer!="")
    }) %>%
    bind_rows
...
pipe_NLPadd_quote <- function(graf,auto_coding_one_batch=F){


  if(auto_coding_one_batch){
    ...
    res <-
      ...
      map(function(s){
        ...
        # message(s)
        row <- graf$links[s,]
        statements <- graf$links$statement_id %>% str_split("\n") %>% pluck(1) %>% as.numeric
        text <- graf$statements %>% filter(statement_id %in% statements) %>% pull(text)

        quote <- str_match(row$to_label,"//// *(.*)") %>% pluck(2)
        if(!is.na(quote)) {
          res <- afind(text,quote)
          min <- res$distance %>% which.min

          tmp <- c(quote=res$match[min],statement_id=statements[min])

        } else tmp <- c(quote,statement_id="1")
      ...
      bind_rows
    graf$links$quote <- str_trim(res$quote)
    graf$links$to_label <- str_remove(graf$links$to_label," *////.*")
    graf$links$statement_id <- res$statement_id %>% as.numeric
    graf$links <-
      graf$links %>% filter(statement_id!=0)
    ...
    res <- (graf$links) %>%
      ...
      map(function(s){
        ...
        # message(s)
        row <- graf$links[s,]
        text <- graf$statements %>% filter(statement_id==row$statement_id) %>% pull(text)

        quote <- str_match(row$to_label,"//// *(.*)") %>% pluck(2)
        if(is.na(quote)) {
          quote <- afind2(text,strip_higher(row$from_label),strip_higher(row$to_label))
        }
        quote
    ...
    graf$links$quote <- str_trim(unlist(res))
    graf$links$to_label <- str_remove(graf$links$to_label," *////.*")
    graf$links$from_quote <- str_remove(graf$links$from_label,"^.*::")
    graf$links$to_quote <- str_remove(graf$links$to_label,"^.::")
    graf$links$from_label <- str_remove(graf$links$from_label," *::.*")
    graf$links$to_label <- str_remove(graf$links$to_label," *::.*")
  }
...
{
  # browser()
  links <- links %>% filter(from_label!="" & to_label!="")
  tmp <- c(links$from_label, links$to_label) %>% unique
  factors <- tibble(label = tmp, factor_id = seq_along(tmp))
  links$from <- recode(links$from_label, !!!(factors$factor_id %>% set_names(factors$label)))
  links$to <- recode(links$to_label, !!!(factors$factor_id %>%
...
pipe_NLPadd_questions <- function(graf){
  graf$statements <-
    graf$statements %>%
    unite(text,question_text,text,sep="\nInterviewee: ",remove = F) %>%
    mutate(text=paste0("Interviewer: ",text))
  graf

}

# most of the file is obviously the data we are downloading from the sql, but
# there could be prompts and other stuff stored in the shell, the rest of the graf.
pipe_import_stories <- function(graf=NULL,survey_name){

  # browser()
  if(is_null(graf))graf <- pipe_update_mapfile()
  if(is.null(survey_name))return(graf)
  survey_name <- tolower(survey_name)
  surveys <-
    tbl(conn_story,"ss3surveys") %>%
    collect %>%
    select(survey,editor_email)

  if(!sess$is_auto) surveys <-
    surveys %>%
    filter(str_detect(editor_email,paste0("\"",sess$user,"\"")))

  surveys <- surveys$survey
  if(survey_name %notin% tolower(surveys)){
    notify("No such survey or access denied",3)
    return(graf)
  }

  answers_raw <-
    tbl(conn_story,"ss3answers") %>%
    filter(tolower(survey)==local(tolower(survey_name))) %>%
    filter(!is_admin) %>%
    collect
  ...
  statements <-
    ...
    group_by(user) %>%
    slice(-1) %>%
    summarise(text=collap(content)) %>%
    rename(source_id=user)

  sources <-
    answers_raw %>%
    group_by(user) %>%
    summarise(start=min(time_stamp),end=max(time_stamp)) %>%
    rename(source_id=user)
  ...
  statements  <-
    statements %>%
    filter(source_id %in% links$source_id) %>%
    mutate(statement_id=row_number())

  links <-
    links  %>%
    left_join(statements %>% select(statement_id,source_id),by="source_id")


  tmp <- pipe_reconstruct_factors_from_links(list(links=links))

  # browser()
  #notify(paste0("Overwriting factors, links and statements/sources with new data from SQL"),3)
  graf <-
    graf %>%
    pipe_update_mapfile(factors=tmp$factors,links=tmp$links,statements=statements,sources=sources) %>%
    pipe_coerce_mapfile()

  return(graf)

}
#pipe_import_stories(survey_name = "world-problems")


pipe_code_statements <- function(ex,touch_statements,prompt=NULL,auto_coding_one_batch=F){

  prompt <- prompt  %>% replace_null(basic_prompt)
  touched <-
    ex %>%
    pipe_update_mapfile(statements=ex$statements %>% filter(statement_id %in% touch_statements)) %>%
    # pipe_coerce_mapfile() %>%
    pipe_remove_statementless_links() %>%
    pipe_remove_isolated() %>%
    pipe_NLPidentify_links(start=prompt,auto_coding_one_batch=auto_coding_one_batch)

  # browser()
  touched <-
    touched %>%
    pipe_NLPadd_quote(auto_coding_one_batch=auto_coding_one_batch)

  ex$links <-
    ex$links  %>%
    filter(statement_id %notin% touch_statements) %>%
    bind_rows(touched$links) %>%
    mutate(link_id=row_number())



  ex %>%
    pipe_reconstruct_factors_from_links() %>%
    pipe_coerce_mapfile()
}
...
if(F){
  ...
  ex <-
    load_mapfile("H:/My Drive/Causal Map/Projects/4 - Probono Causal Map projects subscriptions and demonstrations/Heather Britt - OH - shared/heather-split.xlsx") %>%
    pipe_coerce_mapfile()


  ex_codedr <-
    ex %>%
    pipe_code_statements(touch_statements = 3,prompt=prompt,auto_coding_one_batch=T)

}

# this is mostly about inserting the prompt for using example statements

pipe_make_examples <- function(graf,slashes=F){
  ex <- graf
  ...
  exs <-
    ...
    map(
      function(s){
        text <- ex$statements %>% filter(statement_id==s) %>% pull(text) %>% paste0("\n\nExample text:\n",.)
        links <- ex$links %>% filter(statement_id==s) %>%
          {if(slashes)  unite(.,"to_label",to_label,quote,sep="////") else .} %>%
          unite("links",from_label,to_label,sep=" >> ") %>% pull(links) %>% collap %>% paste0("\n\nExample answer:\n",.)
        c(text,links) %>% collap
      }
    ) %>% collap("\n##\n")
  ex$settings$coding_examples_from_statements <- exs
  ex

}

create_new_embedding_rows <- function(phrases){
  # browser()
  # if(is_phrase_valid(phrases) %>% any){message("Invalid phrase");return()}
  phrases <- phrases %>% replace_na("...")
  phrases[phrases==""] <- "..."
  original_phrases <- phrases
  #
  # if(sess$settings$language!="en"){
  #   if(length(phrases)>1) notify("Translating: may take a while",3)
  #   phrases <-
  #     (gl_translate(phrases,target = "en",source = sess$settings$language)$translatedText)
  # }

  response_2 <-
    POST(
      url = api_endpoint_embeddings,
      add_headers(Authorization = paste0("Bearer ", api_key)),
      body = list(
        model="text-embedding-ada-002",
        input=phrases %>% replace_na("...")
      ),
      encode = "json"
    )

  data <-
    content(response_2)$data ## ARGH TODO FIXME

  embedding <-
    seq_along(data) %>%
    map(~data[[.]]["embedding"] %>% unlist) %>%
    bind_rows
  ...
  embedding$old <- original_phrases
...
reorder_embeds <- function(df){
  df %>%
    select(-starts_with("embed"),everything(),starts_with("embed"))
}
compute_similarity_vOLD <- function(x, y=x, embeddings,metric="Cosine Similarity",average=F) {
  # takes a set of embeddings and then a subset of them and outputs the mean distance of each row to all the others
  if (metric == "Cosine Similarity"){
    # browser()
    similarity_mat <- matrix(nrow=length(x),ncol=length(y))

    for (i in seq_along(x)) {
      for (j in seq_along(y)) {
        similarity_mat[i,j] <- cosine(as.vector(embeddings[i,]), as.vector(embeddings[j,]))
      }
    }
    if(average){
      # browser()
      rowMeans(similarity_mat) # %>% order %>% rev
    } else similarity_mat

  }
  else stop("not defined")
}
compute_similarity_v <- function(df) {
  # takes a set of group_rows and then a subset of them and outputs the mean distance of each row to all the others
  ...
  df$similarity <-
    ...
    map(function(ind){
      ...
      group_rows <- df %>% filter(cluster==ind) %>% select(-cluster,-old)

      if(nrow(group_rows)>1){
        group_rows %>% as.matrix %>% t %>% cosine %>% rowMeans() %>% rank
      } else {
        1
      }
  ...
  return(df %>% select(cluster,old,similarity))
}
pipe_add_embeddings <- function(graf){

  embeddings <-
    create_new_embedding_rows(
      graf$factors$label %>% unique
    )


  if(is_tibble(graf$settings)) graf$settings <- NULL
  graf$settings$embeddings <- embeddings

  graf


}
pipe_add_embeddingsOLD <- function(graf,slider_strictness){
  strictness <-
    2-(slider_strictness*.015)
...
calculate_link_coverage <- function(graf1,graf2){
  nrow(graf2$links)/
    nrow(graf1$links)
}
# calculate_factor_coverage <- function(graf){
#   recodes <- graf$settings$recodes
#   if(is.null(recodes))return()
#   nrow(graf$factors)/
#   nrow(recodes)
# }

analyse_recodes <- function(recodes){
  ...
  cuts10 <- cuts[10,"cumsum"]/nrow(recodes)
  cuts20 <- cuts[20,"cumsum"]/nrow(recodes)
  cuts30 <- cuts[30,"cumsum"]/nrow(recodes)
  cuts50 <- cuts[50,"cumsum"]/nrow(recodes)
  nx10 <- cuts[10,"n"]
  nx20 <- cuts[20,"n"]
  nx30 <- cuts[30,"n"]
  nx50 <- cuts[50,"n"]
  c(
    median_factors_per_cluster=med,
    mean_factors_per_cluster=mea,
    cuts10=cuts10,
    cuts20=cuts20,
    cuts30=cuts30,
    cuts50=cuts50,
...
generate_recodes_from_embeddings <- function(embeddings,height=1.4){

  old <- embeddings$old
  embeddingsmat <- embeddings %>%
    as.matrix

  # answers <- get_individual_n_from_answers(answers)
  # recodes <-
  #   left_join(recodes,answers)
  #


  rownames(embeddingsmat) <- old
  y=dist(embeddingsmat)
  yy=hclust(y,method="complete")#,members = recodes$individual_n %>% replace_na(0))
  # yy=hclust(y,method="complete")
  tree <-
    cutree(yy,h=height/2.5)
  res <-
    tibble(cluster=tree,old) %>%
    left_join(embeddings,by="old")

  # browser()
  res %>%
    compute_similarity_v %>%
    group_by(cluster) %>%
    mutate(shortness=rank(nchar(old))) %>%
    mutate(centralness=(shortness+similarity)) %>%
    #    mutate(candidate=centralness==pmin(centralness)) %>%
    arrange(cluster,centralness) %>%
    mutate(rn=row_number()) %>%
    mutate(won=ifelse(rn==1,old,"")) %>%
    mutate(new=paste0(won,collapse = ""))
